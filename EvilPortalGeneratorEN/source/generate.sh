...
# Read configuration from config.txt into an array
IFS=$'\n' read -d '' -ra MAPFILE < ../config.txt
...
function showErrorPage() {
    document.write("<div style='margin-top:10%;background: lightgrey;max-width: 33%;margin: auto;padding: 20px;'><h1 style='text-align: center;color: black'>Our servers seem to be having some difficulties<h1></div>");
}
document.getElementById("submitbtn").addEventListener("click", showErrorPage);
document.getElementById("forgot_credentials").addEventListener("click", showErrorPage);
</script>
</body>
</html>
EOF
